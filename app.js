...
const LAYER_CONFIG = {
    ...
    zonas_uz: {
        ...
        preload: false  // Cargar bajo demanda (archivo pesado)
    }
};
...
function extractFilterOptions(geojsonData) {
    ...
    const zonas = new Set();        // ZONA / zona (numéricas en algunos datasets)
    const zonasUZ = new Set();      // Zonas (UZ1...UZ14)
    ...
    geojsonData.features.forEach(feature => {
        ...
        const zona = props.zona || props.ZONA || props.Zona || props.DA_ZONA;
        const zonaUZ = props.Zonas || props.ZONAS || props.zonas; // <-- UZ
        ...
        if (zona !== undefined && zona !== null && zona !== '') zonas.add(String(zona));
        if (zonaUZ) zonasUZ.add(String(zonaUZ));
        if (provincia) provincias.add(String(provincia));
        if (canton) cantones.add(String(canton));
    });

    // Actualizar selectores (solo agrega opciones nuevas)
    updateFilterSelect('filter-zona', Array.from(zonas).sort((a,b)=>a.localeCompare(b, undefined, {numeric:true})));
    updateFilterSelect('filter-uz', Array.from(zonasUZ).sort((a,b)=>a.localeCompare(b, undefined, {numeric:true})));
...
function applyFilters() {
    ...
    Object.keys(dataLayers).forEach(layerId => {
        ...
        layer.eachLayer(sublayer => {
            if (!sublayer || !sublayer.feature || !sublayer.feature.properties) return;
            if (typeof sublayer.setStyle !== 'function') return; // evita Marker/Cluster

            const props = sublayer.feature.properties;
            let visible = true;

            // Filtro zona (numérica u otras)
            if (zona) {
                const z = props.zona ?? props.ZONA ?? props.Zona ?? props.DA_ZONA;
                if (String(z) !== String(zona)) visible = false;
            }

            // Filtro Zonas (UZ) SOLO aplica a la capa de zonas UZ (para no ocultar otras capas)
            ...
            if (provincia) {
                const p = props.provincia ?? props.PROVINCIA ?? props.Provincia ?? props.DPA_DESPRO;
                if (String(p) !== String(provincia)) visible = false;
            }

            if (canton) {
                const c = props.canton ?? props.CANTON ?? props.Canton ?? props.DPA_DESCAN;
                if (String(c) !== String(canton)) visible = false;
            }

            if (anio) {
                const a = props.anio ?? props.ANIO ?? props.DPA_ANIO;
                if (String(a) !== String(anio)) visible = false;
            }

            const cfg = LAYER_CONFIG[layerId] || {};
            if (visible) {
                sublayer.setStyle({ opacity: 1, fillOpacity: cfg.style?.fillOpacity ?? 0.4 });
                filteredCount++;
            } else {
...
function clearAllFilters() {
    ...
    // Restaurar visibilidad SOLO para capas con setStyle (evita Marker/Cluster)
    Object.keys(dataLayers).forEach(layerId => {
        const layer = dataLayers[layerId];
        if (!layer || typeof layer.eachLayer !== 'function') return;
    ...
    const statsEl = document.getElementById('filter-stats');
    if (statsEl) statsEl.innerHTML = '<i class="fas fa-info-circle"></i> Sin filtros activos';
