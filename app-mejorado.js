...
// ===== NUEVA FUNCIÓN: PRELOAD ALL LAYERS =====
async function preloadAllLayers() {
    ...
    for (const [layerId, config] of Object.entries(LAYER_CONFIG)) {
        if (config.file) {
            const promise = fetch(config.file)
                .then(response => {
                    if (!response.ok) {
                        throw new Error(`Error al cargar ${config.file}`);
                    }
                ...
                .then(geojsonData => {
                    ...
                    return { layerId, success: true, count: geojsonData.features.length };
                })
                .catch(error => {
                    console.error(`Error cargando ${layerId}:`, error);
                    return { layerId, success: false, error: error.message };
                });
    ...
    try {
        ...
        results.forEach(result => {
            if (result.success) {
                updateLayerCount(result.layerId, result.count);
            }
        });
        
        updateLegend();
        const successCount = results.filter(r => r.success).length;
        showToast(`${successCount} capas cargadas exitosamente`, 'success');
        
    } catch (error) {
        showToast('Error al cargar algunas capas', 'error');
    } finally {
        showLoading(false);
    }
}

// ===== CREAR CAPA DESDE DATOS PRECARGADOS =====
...
function toggleLayer(layerId, visible) {
    if (!dataLayers[layerId]) {
        showToast('Capa aún no cargada', 'warning');
        return;
    }
    ...
    if (visible) {
        ...
        showToast(`${LAYER_CONFIG[layerId].name} activada`, 'info');
    } else {
        map.removeLayer(dataLayers[layerId]);
        document.getElementById(`card-${layerId}`).classList.remove('active');
        showToast(`${LAYER_CONFIG[layerId].name} desactivada`, 'info');
    }
