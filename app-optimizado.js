...
let provinciaStatsByName = {}; // stats por provincia (Cuadro 1.5 REBPE 2023)
let provinciaStatsLoaded = false;
...
async function preloadAllLayers() {
    ...
    // Cargar estadísticas provinciales (Cuadro 1.5) antes de generar popups
    showLoading(true, 'Cargando estadísticas provinciales...');
    await loadProvinciaStats();
...
// ======================================================
// Estadísticas provinciales (Cuadro 1.5 - REBPE 2023)
// Archivo esperado: stats_nacionalidad_provincia_solo_provincias.csv
// ======================================================
...
// Parser CSV simple (suficiente para nuestros datos numéricos)
...
async function loadProvinciaStats() {
    if (provinciaStatsLoaded) return true;

    try {
        const res = await fetch('stats_nacionalidad_provincia_solo_provincias.csv', { cache: 'no-store' });
        if (!res.ok) throw new Error(`No se pudo cargar stats: HTTP ${res.status}`);
        ...
        rows.forEach(r => {
            const provKey = normalizeKey(r.provincia);
            if (!provKey) return;

            const ecu_h = toIntSafe(r.ecuatoriana_hombre);
            const ecu_m = toIntSafe(r.ecuatoriana_mujer);
            const ecu_i = toIntSafe(r.ecuatoriana_indefinido);

            const ext_h = toIntSafe(r.extranjera_hombre);
            const ext_m = toIntSafe(r.extranjera_mujer);
            const ext_i = toIntSafe(r.extranjera_indefinido);

            const nr_h = toIntSafe(r.no_registra_hombre);
            const nr_m = toIntSafe(r.no_registra_mujer);
            const nr_i = toIntSafe(r.no_registra_indefinido);

            const ecu_t = ecu_h + ecu_m + ecu_i;
            const ext_t = ext_h + ext_m + ext_i;
            const nr_t  = nr_h + nr_m + nr_i;
            const total = ecu_t + ext_t + nr_t;

            mapStats[provKey] = {
                provincia: r.provincia,
                anio: r.anio || '',
                fuente: r.fuente || '',
                ecuatoriana: { h: ecu_h, m: ecu_m, i: ecu_i, t: ecu_t },
                extranjera:  { h: ext_h, m: ext_m, i: ext_i, t: ext_t },
                no_registra: { h: nr_h, m: nr_m, i: nr_i, t: nr_t },
                total
            };
        });

        provinciaStatsByName = mapStats;
        provinciaStatsLoaded = true;
        console.log('✓ Estadísticas provinciales cargadas:', Object.keys(provinciaStatsByName).length);
        return true;
    } catch (e) {
        console.warn('⚠️ No se pudo cargar stats provinciales:', e);
        provinciaStatsLoaded = false;
        provinciaStatsByName = {};
        return false;
    }
}

function getProvinciaStatsForFeature(props) {
    const provName = props?.DPA_DESPRO || props?.provincia || props?.PROVINCIA || '';
    const key = normalizeKey(provName);
    return provinciaStatsByName[key] || null;
}

function fmtInt(n) {
    try { return Number(n || 0).toLocaleString('es-EC'); } catch { return String(n || 0); }
}

function buildProvinciaStatsHTML(stats) {
    if (!stats) {
        return `
            <div class="popup-grid">
                <div class="kv">
                    <div class="k">Estadística</div>
                    <div class="v">Sin datos (verifica el CSV)</div>
                </div>
            </div>
        `;
    }
    ...
    return `
        <div class="popup-badges">
            <span class="badge badge-zone">Año ${escapeHtml(stats.anio || '2023')}</span>
        </div>

        <div class="popup-grid">
            <div class="kv">
                <div class="k">Total población</div>
                <div class="v">${fmtInt(stats.total)}</div>
            </div>

            <div class="kv">
                <div class="k">Ecuatoriana</div>
                <div class="v">${fmtInt(stats.ecuatoriana.t)} <span style="opacity:.75">(${fmtInt(stats.ecuatoriana.h)} H · ${fmtInt(stats.ecuatoriana.m)} M · ${fmtInt(stats.ecuatoriana.i)} I)</span></div>
            </div>

            <div class="kv">
                <div class="k">Extranjera</div>
                <div class="v">${fmtInt(stats.extranjera.t)} <span style="opacity:.75">(${fmtInt(stats.extranjera.h)} H · ${fmtInt(stats.extranjera.m)} M · ${fmtInt(stats.extranjera.i)} I)</span></div>
            </div>

            <div class="kv">
                <div class="k">No registra</div>
                <div class="v">${fmtInt(stats.no_registra.t)} <span style="opacity:.75">(${fmtInt(stats.no_registra.h)} H · ${fmtInt(stats.no_registra.m)} M · ${fmtInt(stats.no_registra.i)} I)</span></div>
            </div>
        </div>

        ${stats.fuente ? `<div style="padding: 0 12px 10px 12px; font-size: 11px; color: rgba(255,255,255,0.65);">Fuente: ${escapeHtml(stats.fuente)}</div>` : ''}
    `;
}
...
function createDynamicPopup(feature, layerId) {
    ...
    // Popup especial para Provincias: incluye estadísticas (Cuadro 1.5 REBPE 2023)
    if (layerId === 'provincia') {
        const provName = props.DPA_DESPRO || props.PROVINCIA || props.provincia || 'Provincia';
        const provCode = props.DPA_PROVIN || '';
        const stats = getProvinciaStatsForFeature(props);

        return `
            <div class="popup-card popup-provincia">
                <div class="popup-header">
                    <div class="popup-title">${escapeHtml(String(provName))}</div>
                    <div class="popup-subtitle">${provCode ? `Código: ${escapeHtml(String(provCode))}` : ''}</div>
                </div>

                ${buildProvinciaStatsHTML(stats)}
...
function showFeatureInfo(feature, layerId) {
    ...
    // Para Provincias: incluir estadísticas (Cuadro 1.5 REBPE 2023)
    if (layerId === 'provincia') {
        const provName = props.DPA_DESPRO || props.PROVINCIA || props.provincia || 'Provincia';
        const provCode = props.DPA_PROVIN || '';
        const stats = getProvinciaStatsForFeature(props);
        ...
        content += `
            <div class="info-section">
                <h5><i class="fas fa-chart-bar"></i> Estadística por provincia</h5>
                <div class="info-row"><strong>Provincia:</strong><span>${escapeHtml(String(provName))}</span></div>
                ${provCode ? `<div class="info-row"><strong>Código:</strong><span>${escapeHtml(String(provCode))}</span></div>` : ''}
                ${stats ? `
                    <div class="info-row"><strong>Total población:</strong><span>${fmtInt(stats.total)}</span></div>
                    <div class="info-row"><strong>Ecuatoriana:</strong><span>${fmtInt(stats.ecuatoriana.t)} (H ${fmtInt(stats.ecuatoriana.h)} · M ${fmtInt(stats.ecuatoriana.m)} · I ${fmtInt(stats.ecuatoriana.i)})</span></div>
                    <div class="info-row"><strong>Extranjera:</strong><span>${fmtInt(stats.extranjera.t)} (H ${fmtInt(stats.extranjera.h)} · M ${fmtInt(stats.extranjera.m)} · I ${fmtInt(stats.extranjera.i)})</span></div>
                    <div class="info-row"><strong>No registra:</strong><span>${fmtInt(stats.no_registra.t)} (H ${fmtInt(stats.no_registra.h)} · M ${fmtInt(stats.no_registra.m)} · I ${fmtInt(stats.no_registra.i)})</span></div>
                    ${stats.fuente ? `<div class="info-row"><strong>Fuente:</strong><span>${escapeHtml(stats.fuente)}</span></div>` : ''}
                ` : `<div class="info-row"><strong>Estadística:</strong><span>Sin datos (verifica el CSV)</span></div>`}
            </div>
        `;
    }
