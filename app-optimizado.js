...
function getRegimenFromProvincia(provincia) {
    const p = normalizeText(provincia);
    const costaGalapagos = new Set([
        'ESMERALDAS','MANABI','LOS RIOS','GUAYAS','SANTA ELENA','EL ORO',
        'SANTO DOMINGO DE LOS TSACHILAS','GALAPAGOS'
    ]);
    return costaGalapagos.has(p) ? 'Costa-Galápagos' : 'Sierra-Amazonía';
}

// Zonas (Tabla 1 de la metodología) – etiqueta + cobertura referencial
const ZONA_APIT_INFO = {
    1: { nombre: 'Unidad Desconcentrada Zonal 1', cobertura: 'Carchi, Esmeraldas, Imbabura y Sucumbíos' },
    2: { nombre: 'Unidad Desconcentrada Zonal 2', cobertura: 'Napo, Orellana y Pichincha (excepto DMQ)' },
    3: { nombre: 'Unidad Desconcentrada Zonal 3', cobertura: 'Cotopaxi, Chimborazo, Pastaza y Tungurahua' },
    4: { nombre: 'Unidad Desconcentrada Zonal 4', cobertura: 'Manabí y Santo Domingo de los Tsáchilas' },
    5: { nombre: 'Unidad Desconcentrada Zonal 5', cobertura: 'Bolívar, Guayas (excepto Guayaquil/Samborondón/Durán), Los Ríos, Galápagos y Santa Elena' },
    6: { nombre: 'Unidad Desconcentrada Zonal 6', cobertura: 'Azuay, Cañar y Morona Santiago' },
    7: { nombre: 'Unidad Desconcentrada Zonal 7', cobertura: 'El Oro, Loja y Zamora Chinchipe' },
    8: { nombre: 'Subsecretaría Zonal de Educación de Guayaquil', cobertura: 'Guayaquil, Samborondón y Durán' },
    9: { nombre: 'Subsecretaría Zonal de Educación del Distrito Metropolitano de Quito', cobertura: 'Distrito Metropolitano de Quito' }
};

function zonaApitNombre(z) {
    const n = Number(z);
    return ZONA_APIT_INFO[n]?.nombre || 'Zona';
}

function zonaApitCobertura(z) {
    const n = Number(z);
    return ZONA_APIT_INFO[n]?.cobertura || '';
}
...
function enrichCantonesGeojson(geojson) {
    if (!geojson || !geojson.features) return geojson;
    geojson.features.forEach(f => {
        const p = f.properties || {};
        const prov = p.DPA_DESPRO || p.provincia || p.PROVINCIA || '';
        const zonaNum = (p.ZONA ?? p.zona ?? '');
        p.REGIMEN = p.REGIMEN || getRegimenFromProvincia(prov);
        p.ZONA_NOMBRE = p.ZONA_NOMBRE || zonaApitNombre(zonaNum);
        p.ZONA_COBERTURA = p.ZONA_COBERTURA || zonaApitCobertura(zonaNum);
        p.NMT_25_DESC = p.NMT_25_DESC || nmt25Label(p.NMT_25 || p.nmt25);
        f.properties = p;
    });
    return geojson;
}
...
async function preloadAllLayers() {
    ...
    for (const [layerId, config] of layersToPreload) {
        ...
        try {
            ...
            // Enriquecer cantones para filtros/popup (metodología)
            const processedData = (layerId === 'cantones') ? enrichCantonesGeojson(geojsonData) : geojsonData;

            // Guardar datos
            layerData[layerId] = processedData;

            // Crear capa (pero NO agregarla al mapa aún)
            createLayerFromData(layerId, processedData);

            // KPI OT/UZ/UD (Cantones NMTD)
            if (layerId === 'cantones' && processedData && Array.isArray(processedData.features)) {
                lastCantonesFilteredFeatures = processedData.features;
                updateNmt25Kpis(processedData.features);
            }
...
function createDynamicPopup(feature, layerId) {
    ...
    // Para Cantones NMTD: mostrar clasificación OT/UZ/UD en claro
    let rowsPrefix = '';
    if (layerId === 'cantones') {
        const code = normalizeNmt25(props?.NMT_25);
        const label = nmt25Label(code);
        rowsPrefix = `
            <div class="popup-row">
                <div class="popup-label">Tipo:</div>
                <div class="popup-value">${escapeHtml(label)}${code ? ` (${escapeHtml(code)})` : ''}</div>
            </div>
        `;
    }
...
function applyCantonesPointFilter({ nmt25, provincia, canton, zona, regimen }) {
...
function clearAllFilters() {
    ...
    const elNmt25 = document.getElementById('filter-nmt25');
    const elRegimen = document.getElementById('filter-regimen');
    ...
    if (elUZ) elUZ.value = '';
    if (elNmt25) elNmt25.value = '';
    if (elRegimen) elRegimen.value = '';
